...
<div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 25px;">
    <div class="stat-card">
        ...
        <div style="font-size: 24px; font-weight: 700; margin-top: 5px;">{{ $collections->total() }}</div>
    </div>
    <div class="stat-card">
        <div style="font-size: 13px; color: #999;">Đang hoạt động</div>
        <div style="font-size: 24px; font-weight: 700; color: #28a745; margin-top: 5px;">{{ $collections->where('status', 1)->count() }}</div>
    </div>
    <div class="stat-card">
        <div style="font-size: 13px; color: #999;">Bản nháp</div>
        <div style="font-size: 24px; font-weight: 700; color: #999; margin-top: 5px;">{{ $collections->where('status', 0)->count() }}</div>
    </div>
    <div class="stat-card">
        <div style="font-size: 13px; color: #999;">Tổng sản phẩm</div>
        <div style="font-size: 24px; font-weight: 700; color: #1976d2; margin-top: 5px;">{{ $collections->sum('products_count') }}</div>
    </div>
</div>
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.collections.index') }}" method="GET" style="display: flex; gap: 10px; margin-bottom: 20px;">
            <input type="text" name="keyword" class="form-control" placeholder="Tìm bộ sưu tập..." value="{{ request('keyword') }}" style="max-width: 300px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px;">
            ...
            <div style="border: 1px solid #f0f0f0; border-radius: 12px; overflow: hidden;">
                <div style="height: 160px; overflow: hidden; background: #f5f5f5;">
                    @if($col->banner_url)
                        <img src="{{ asset('storage/' . $col->banner_url) }}" style="width: 100%; height: 100%; object-fit: cover;">
                    @else
                ...
                <div style="padding: 15px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                        <h4 style="font-size: 16px; font-weight: 600;">{{ $col->name }}</h4>
                        @if($col->status)
                    ...
                    <p style="color: #999; font-size: 13px; margin-bottom: 12px;">{{ $col->products_count }} sản phẩm · {{ $col->created_at->format('d/m/Y') }}</p>
                    <div style="display: flex; gap: 8px;">
                        <a href="{{ route('admin.collections.edit', $col->id) }}" class="btn btn-sm btn-primary" style="flex: 1; text-align: center;">
                            <i class="fa-solid fa-pen"></i> Sửa
                        </a>
                        <form action="{{ route('admin.collections.destroy', $col->id) }}" method="POST" onsubmit="return confirm('Xóa bộ sưu tập này?')">
                            @csrf @method('DELETE')
        ...
        <div style="margin-top: 15px;">{{ $collections->appends(request()->query())->links() }}</div>
    </div>
</div>
